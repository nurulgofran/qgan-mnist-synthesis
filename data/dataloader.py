...
class EMNISTQuantumDataset(Dataset):
    def __init__(
        ...
        print(f"Filtered to {len(df)} samples for digits {target_digits}")

        if max_samples_per_digit:
            df = df.groupby("label").head(max_samples_per_digit)
            print(f"Limited to {max_samples_per_digit} samples/digit: {len(df)} total")
        ...
        print(
            f"Dataset ready: {len(self.images)} images of shape {self.images[0].shape}"
        )
